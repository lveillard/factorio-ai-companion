...
commands.add_command("fac_api", "AI Companion structured API", function(cmd)
  u.safe_command(function()
    ...
    local definition = contract[request.tool]
    local handler = u.handlers[request.tool]
    if not definition or definition.execution ~= "game" or not handler then error("Unknown tool: " .. request.tool) end
    local args = request.args or {}
    validate(args, definition.inputSchema, request.tool)
